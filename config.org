...
#+begin_src emacs-lisp
  ...
  (defun visd0m/git-remote-url ()
    ...
    (with-temp-buffer
      ...
      (vc-git--call nil (current-buffer) "config" "--get" "remote.origin.url")
...
To have multiple sqli buffer do as follow: C-u M-x sql-postgres
...
** Test microphone input (ubuntu)

Creates a loopback module that connects your input directly to your output, and the unload the module

#+begin_src sh
# to enable the loopback module
pactl load-module module-loopback latency_msec=1

# to unload the loopback module
pactl unload-module module-loopback
#+end_src

** Env vars to setup claude code to interact with local llm installed and served via ollama
#+begin_src sh
